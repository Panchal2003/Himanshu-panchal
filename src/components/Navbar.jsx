...
function Navbar() {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between p-3 sm:p-4 lg:max-w-7xl lg:mx-auto lg:px-8">
        <a href="#home" className="flex items-center gap-2 sm:gap-3 text-slate-900 shrink-0">
          <div className="h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-premiumGold flex items-center justify-center shadow-soft">
            <span className="text-premiumBlue font-bold text-sm sm:text-lg">B</span>
          </div>
          <div className="hidden sm:block">
            <p className="font-bold text-sm sm:text-lg tracking-wide">B Financial Solutions</p>
        ...
        <button
          className="block rounded-md p-1.5 sm:p-2 text-slate-700 lg:hidden shrink-0"
        ...
        <nav className={`absolute top-full left-0 right-0 bg-white/95 backdrop-blur-xl border-b border-slate-200 shadow-lg lg:relative lg:top-auto lg:left-auto lg:right-auto lg:bg-transparent lg:border-none lg:shadow-none lg:flex lg:items-center ${open ? 'block' : 'hidden'}`}>
          <ul className="flex flex-col gap-1 p-3 text-xs sm:text-sm text-slate-700 lg:flex-row lg:gap-4 xl:gap-6 lg:p-0 lg:items-center">
            {navItems.map((item) => {
              const Icon = item.icon;
              return (
                <li key={item.href}>
                  <a href={item.href} className="flex items-center gap-1.5 sm:gap-2 hover:text-premiumGold transition-all duration-300 py-2 lg:py-0" onClick={() => setOpen(false)}>
